...
class UserService {
    ...
    static getInfoUser = async ({ username }) => {
        ...
        const user = getInfoData({ fields: ['_id', 'username', 'name', 'email', 'phone', 'avatar', 'address'], object: foundUser});
        return user;
    }

    static updateInfoUser = async (id, payload) => {
        const updatedUser = await updateUserById({ id, payload });
        const user = getInfoData({ fields: ['_id', 'username', 'name', 'email', 'phone', 'avatar', 'address'], object: updatedUser});
        return user;
    }
